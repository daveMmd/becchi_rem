...
state_t EgCmpDfa::getNext(state_t current, unsigned char input) {
 	...
 	switch(this->types[current]) {
 		case 1:
			...
			data1 = (EgCmpDfaRleRangeItem*)((EgCmpDfaEdage *)this->edges[current])->data;
 			if(data1->end==input) {
 				return data1->target;
 			}
 			break;
 		case 2:
			...
			data23 = (EgCmpDfaHasBitEdage*)((EgCmpDfaEdage *)this->edges[current])->data;
 			//for(; i < this->edges[current].lenght; ++i ) {
			for(; i < ((EgCmpDfaEdage *)this->edges[current])->lenght; ++i ) {
 		...
 		case 4:
			...
			data4 = (EgCmpDfaSampleRange*)((EgCmpDfaEdage *)this->edges[current])->data;
 			//for(; i < this->edges[current].lenght; ++i ) {
 			for(; i < ((EgCmpDfaEdage *)this->edges[current])->lenght; ++i ) {
 				if(input >= data4[i].start && input <= data4[i].end) {
 					return data4[i].target;
 		...
 		case 5:
 			...
 			data5 = (EgCmpDfaRleRange*)((EgCmpDfaEdage *)this->edges[current])->data;
 			if (BitmapCheck(data5->bitmap,CHAR_BIRMAP_SIZE, input)){
 				//for(; i < this->edges[current].lenght; ++i ) {
 				for(; i < ((EgCmpDfaEdage *)this->edges[current])->lenght; ++i ) {
 					if(input <= data5->ranges[i].end) {
 		...
 		case 6:
 			...
 			data6 = (EgCmpDfaBitmapRange*)((EgCmpDfaEdage *)this->edges[current])->data;
 			//for(; i < this->edges[current].lenght; ++i ) {
 			for(; i < ((EgCmpDfaEdage *)this->edges[current])->lenght; ++i ) {
